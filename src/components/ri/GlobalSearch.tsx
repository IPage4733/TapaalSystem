...
const GlobalSearch: React.FC = () => {
  ...
  // Get VROs under RI for scope filtering
  ...
  const searchResults = useMemo(() => {
    ...
    // Search in tappals (scope limited to RI's jurisdiction)
    if (searchType === 'all' || searchType === 'tappal') {
      const officerIds = [...vrosUnderRI.map(v => v.id), user?.id].filter(Boolean);
      const jurisdictionTappals = mockTappals.filter(t => officerIds.includes(t.assignedTo));
      
      jurisdictionTappals.forEach(tappal => {
        if (
    ...
    // Search in users (scope limited to VROs)
    if (searchType === 'all' || searchType === 'user') {
      vrosUnderRI.forEach(vro => {
        if (
          vro.name.toLowerCase().includes(term) ||
          vro.email.toLowerCase().includes(term) ||
          vro.phoneNumber.includes(term) ||
          vro.department.toLowerCase().includes(term)
        ) {
          results.push({
            type: 'user',
            data: vro,
            matchField: getUserMatchField(vro, term)
          });
        }
      });
    }
    ...
    return results;
  }, [searchTerm, searchType, vrosUnderRI, user]);
  ...
  const getUserMatchField = (vro: any, term: string) => {
    if (vro.name.toLowerCase().includes(term)) return 'Name';
    if (vro.email.toLowerCase().includes(term)) return 'Email';
    if (vro.phoneNumber.includes(term)) return 'Phone';
    return 'Department';
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <p className="text-gray-600">Search across tappals, VROs, and departments in your jurisdiction</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by Tappal ID, Petition ID, VRO Name, Phone Number, Department..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent text-lg"
            />
          </div>

          {/* Search Tips */}
          <div className="bg-teal-50 rounded-lg p-4">
            <h3 className="text-sm font-medium text-teal-800 mb-2">Search Tips:</h3>
            <ul className="text-sm text-teal-700 space-y-1">
              <li>• Search by Tappal ID (e.g., TAP-2025-001)</li>
              <li>• Search by Petition ID (e.g., PET-2025-001)</li>
              <li>• Search by VRO name or phone number</li>
              <li>• Search by department name or code</li>
              <li>• Use partial matches for broader results</li>
              <li>• Search scope limited to your jurisdiction (RI + VROs)</li>
            </ul>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {searchResults.length > 0 && (
              <span className="text-sm text-gray-500">
                Showing results for "{searchTerm}" in your jurisdiction
              </span>
            )}
          </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          <span className="text-xs font-medium text-teal-600 uppercase">Tappal</span>
                          <span className="text-xs text-gray-500">Match in: {result.matchField}</span>
                        </div>
                        <button
                          onClick={() => handleTappalClick(result.data.tappalId)}
                          className="text-lg font-medium text-gray-900 hover:text-teal-600 flex items-center space-x-1"
                        >
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          <span className="text-xs font-medium text-blue-600 uppercase">VRO</span>
                          <span className="text-xs text-gray-500">Match in: {result.matchField}</span>
                        </div>
                        <h3 className="text-lg font-medium text-gray-900">{result.data.name}</h3>
                        <p className="text-gray-600 mt-1">Village Revenue Officer</p>
                        <div className="flex items-center space-x-6 mt-3 text-sm text-gray-500">
                          <div className="flex items-center space-x-1">
