...
const MyTappals: React.FC = () => {
  ...
  const handleForward = (tappalId: string) => {
    navigate(`/ri-dashboard/forward-tappal?tappal=${tappalId}`);
  };

  const handleMarkCompleted = (tappalId: string) => {
    showToast({
      type: 'success',
      title: 'Tappal Marked as Completed',
      message: `${tappalId} has been marked as completed successfully.`,
      duration: 4000
    });
  };
  ...
  const handleUploadAttachment = (tappalId: string) => {
    navigate(`/ri-dashboard/attachments?tappal=${tappalId}`);
  };

  const handleAddRemark = (tappalId: string) => {
    showToast({
      type: 'info',
      title: 'Add Remark',
      message: `Opening remark editor for ${tappalId}. This feature will be available soon.`,
      duration: 4000
    });
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <p className="text-gray-600">Tappals assigned directly to you as Revenue Inspector</p>
      </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {tappal.attachments && tappal.attachments.length > 0 ? (
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={() => handleUploadAttachment(tappal.tappalId)}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center space-x-2">
                        <button
                          ...
                          className="inline-flex items-center px-3 py-1 border border-teal-300 rounded-lg text-xs font-medium text-teal-700 bg-teal-50 hover:bg-teal-100 transition-colors"
                        >
                          <Send className="h-3 w-3 mr-1" />
                          Forward
                        </button>
                        <button
                          onClick={() => handleAddRemark(tappal.tappalId)}
                        ...
                        {tappal.status !== 'Completed' && (
                          <button
                            onClick={() => handleMarkCompleted(tappal.tappalId)}
