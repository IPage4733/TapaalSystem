...
const MyTappals: React.FC = () => {
  ...
  // Get tappals assigned to current DRO (logged-in user)
  const myTappals = useMemo(() => {
    if (!user?.id) return [];
    return allTappals.filter(t => t.assignedTo === user.id);
  }, [allTappals, user]);

  // Status options derived from myTappals (+ common ones)
  const statusOptions = useMemo(() => {
    const s = new Set<string>();
    myTappals.forEach(t => { if (t.status) s.add(t.status); });
    ['Pending', 'In Progress', 'Under Review', 'Completed', 'Rejected', 'FORWARDED', 'Active']
      .forEach(x => s.add(x));
    return Array.from(s);
  }, [myTappals]);

  // Department suggestions (for info – input stays free text)
  ...
  const filteredTappals = useMemo(() => {
    return myTappals.filter(tappal => {
      const matchesStatus =
        !filters.status || (tappal.status || '').toLowerCase() === filters.status.toLowerCase();
      const deptText = (tappal.departmentName || tappal.department || '').toLowerCase();
      const matchesDepartment =
        !filters.department || deptText.includes(filters.department.toLowerCase());
  ...
  const handleForward = (tappalId: string) => {
    navigate(`/dro-dashboard/forward-tappal?tappal=${tappalId}`);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <p className="text-gray-600">Tappals assigned directly to you as DRO</p>
      </div>
