...
const SubordinateTappals: React.FC = () => {
  ...
  const filteredTappals = useMemo(() => {
    return subordinateTappals.filter(tappal => {
      ...
      if (filters.expiryStatus === 'overdue') {
        matchesExpiryStatus = isOverdue(tappal.expiryDate, tappal.status);
      } else if (filters.expiryStatus === 'due-soon') {
        const daysToExpiry = Math.ceil((new Date(tappal.expiryDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
        matchesExpiryStatus = daysToExpiry <= 3 && daysToExpiry >= 0;
      }

      return matchesOfficer && matchesStatus && matchesExpiryStatus;
    });
  ...
  // Calculate stats
  const stats = useMemo(() => {
    const total = subordinateTappals.length;
    const pending = subordinateTappals.filter(t => t.status === 'Pending').length;
    const inProgress = subordinateTappals.filter(t => t.status === 'In Progress').length;
    const completed = subordinateTappals.filter(t => t.status === 'Completed').length;
    const overdue = subordinateTappals.filter(t => isOverdue(t.expiryDate, t.status)).length;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Subordinate Tappals (RI/VRO)</h1>
        <p className="text-gray-600">Monitor tappals assigned to Revenue Inspectors and Village Revenue Officers</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">
            Subordinate Tappals ({filteredTappals.length})
          </h2>
        </div>
...
export default SubordinateTappals;