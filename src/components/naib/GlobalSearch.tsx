...
const GlobalSearch: React.FC = () => {
  ...
  // Get officers below Naib Tahsildar for scope filtering
  ...
  const searchResults = useMemo(() => {
    ...
    // Search in tappals (scope limited to Naib's span)
    if (searchType === 'all' || searchType === 'tappal') {
      const officerIds = [...officersBelow.map(o => o.id), user?.id].filter(Boolean);
      const spanTappals = mockTappals.filter(t => officerIds.includes(t.assignedTo));
      
      spanTappals.forEach(tappal => {
        if (
    ...
    // Search in users (scope limited to subordinates)
    if (searchType === 'all' || searchType === 'user') {
      officersBelow.forEach(officer => {
        if (
          ...
          officer.phoneNumber.includes(term) ||
          officer.department.toLowerCase().includes(term)
        ) {
          results.push({
            type: 'user',
            data: officer,
            matchField: getUserMatchField(officer, term)
          });
        }
      });
    }
    ...
    return results;
  }, [searchTerm, searchType, officersBelow, user]);
  ...
  const getUserMatchField = (officer: any, term: string) => {
    if (officer.name.toLowerCase().includes(term)) return 'Name';
    if (officer.email.toLowerCase().includes(term)) return 'Email';
    if (officer.phoneNumber.includes(term)) return 'Phone';
    return 'Department';
  };
  ...
  const searchTypeOptions = [
    ...
    { value: 'user', label: 'Officers Only' },
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <p className="text-gray-600">Search across tappals, officers, and departments in your span</p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-lg"
            />
          </div>

          {/* Search Tips */}
          <div className="bg-emerald-50 rounded-lg p-4">
            <h3 className="text-sm font-medium text-emerald-800 mb-2">Search Tips:</h3>
            <ul className="text-sm text-emerald-700 space-y-1">
              ...
              <li>• Search scope limited to your span (RI, VRO, Clerk)</li>
            </ul>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {searchResults.length > 0 && (
              <span className="text-sm text-gray-500">
                Showing results for "{searchTerm}" in your span
              </span>
            )}
          </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          <span className="text-xs font-medium text-emerald-600 uppercase">Tappal</span>
                          <span className="text-xs text-gray-500">Match in: {result.matchField}</span>
                        </div>
                        <button
                          onClick={() => handleTappalClick(result.data.tappalId)}
                          className="text-lg font-medium text-gray-900 hover:text-emerald-600 flex items-center space-x-1"
                        >
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          <span className="text-xs font-medium text-blue-600 uppercase">Officer</span>
                          <span className="text-xs text-gray-500">Match in: {result.matchField}</span>
                        </div>
                        <h3 className="text-lg font-medium text-gray-900">{result.data.name}</h3>
                        <p className="text-gray-600 mt-1">{getRoleDisplayName(result.data.role)}</p>
                        <div className="flex items-center space-x-6 mt-3 text-sm text-gray-500">
                          <div className="flex items-center space-x-1">
