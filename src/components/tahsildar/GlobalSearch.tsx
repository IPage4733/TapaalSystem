...
const GlobalSearch: React.FC = () => {
  ...
  // Get officers below Tahsildar for scope filtering
  ...
  const searchResults = useMemo(() => {
    ...
    // Search in tappals (scope limited to mandal)
    if (searchType === 'all' || searchType === 'tappal') {
      const officerIds = officersBelow.map(o => o.id);
      const mandalTappals = mockTappals.filter(t => officerIds.includes(t.assignedTo));
      
      mandalTappals.forEach(tappal => {
        if (
    ...
    return results;
  }, [searchTerm, searchType, officersBelow]);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <p className="text-gray-600">Search across tappals, officers, and departments in your mandal</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <div className="space-y-4">
          ...
          <div className="bg-green-50 rounded-lg p-4">
            <h3 className="text-sm font-medium text-green-800 mb-2">Search Tips:</h3>
            <ul className="text-sm text-green-700 space-y-1">
              ...
              <li>• Search scope limited to your mandal officers</li>
            </ul>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {searchResults.length > 0 && (
              <span className="text-sm text-gray-500">
                Showing results for "{searchTerm}" in your mandal
              </span>
            )}
          </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          <span className="text-xs font-medium text-green-600 uppercase">Tappal</span>
                          <span className="text-xs text-gray-500">Match in: {result.matchField}</span>
                        </div>
                        <button
                          onClick={() => handleTappalClick(result.data.tappalId)}
                          className="text-lg font-medium text-gray-900 hover:text-green-600 flex items-center space-x-1"
                        >
